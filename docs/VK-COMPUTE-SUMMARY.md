...
7. **(Resolved) End-to-end coherence, prefill and decode.** The engine now produces coherent, extended output against the real 9B weights. A layer-by-layer differential (`--dump` + `cmp_layers.py`, run under `tools/pruner/.venv`) shows all 32 prefill layers tracking the HF reference at 0.96–0.9999 cosine correlation, and the decode trajectory matches the pruned-vocab-constrained HF greedy token-for-token. Seven bugs were fixed across prefill and decode (gotchas 27–32): the headline prefill bug was the full-attention output gate (`sigmoid` vs `silu`, gotcha 27), and the headline decode bug was `Att-full-{INT8,INT4}` dequantizing V with K's scale/zero (gotcha 31). The chat-template `<think>` token fix (gotcha 32) was what finally enabled thinking-mode output. Remaining gap: the INT4/INT8 quantization error still accumulates over the deep layers and can loop/degrade on long generations — a quantization-quality issue, not a correctness bug.
