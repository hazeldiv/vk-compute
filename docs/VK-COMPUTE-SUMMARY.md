...
| `vocab` | 86016 | 86016 | `quant_config.json` `vocab_size` (pruned; EOS derived from vocab files) |
...
vk-compute/
       ...
       tools/setup_2b.py            # 2B prep: checks tokenizer parity, runs the pruner
       ...
       model/
            Qwen3.5-9B-weight/      # 9B: shards + config.json + quant_config.json + vocab/
...
model/<name>/
       ...
       quant_config.json                 # engine config: name, vocab_size, max_ctx, prefill_chunk,
...
1. `pruneVocab(modelDir, sourceDir)` (src/prune.c, gated by `--prune <vocab_dir>`): if `<modelDir>/vocab/embed_tokens.*.safetensors` doesn't exist, reads the pruner's `mapping.npy` (int32[V], new-id — original-id; the npy header's version bytes must be skipped — gotcha 36), gathers the V embedding rows from the shard (`row v` = source row `mapping[v]`), writes `vocab/embed_tokens.<V>.safetensors` (BF16, proper safetensors header), and copies `tokenizer.json`/`tokenizer_config.json`/`vocab.json`/`mapping.npy` into `vocab/`. Idempotent (skips if already pruned).
2. `loadModelConfig` (src/model.c) parses `<modelDir>/config.json` (HF: hidden_size, layer count, layer_types, head counts, head_dim, intermediate_size, linear_* geometry, rope_parameters, tie_word_embeddings) with the generic JSON DOM parser (src/json.c) and derives `qkvN`, `projN`, `zqkvN`, `kvRows`, offsets, `rotaryDim`. It then parses `quant_config.json` (name, vocab_size, max_ctx, prefill_chunk, per-layer attn/ffn quant, embed/lm_head quant) and derives **EOS from the vocab files** — `tokenizer_config.json`'s `eos_token` name looked up in `vocab.json` (handles both plain-string and `{content:   }` forms) — so EOS can never drift from the tokenizer (gotcha 35: a hardcoded id from a stale header once made generation never stop).
3. Weight tensor discovery globs `model.safetensors*.safetensors` (any shard count) instead of hardcoding 4 shards.
...
5. Small vectors (norms, `A_log`, `dt_bias`, `conv1d`) have their own on-disk cache (`vec_<label>_<layer>.bin`, `conv_<layer>.bin`) so a cache-only run needs no safetensors at all.
...
.venv/Scripts/python.exe vk_llm.py <weight_dir> <max_ctx> ["prompt text"] [thinking|none] [prune_vocab_dir]
# e.g.
.venv/Scripts/python.exe vk_llm.py model/Qwen3.5-2B 8192 "why the sky is blue?" thinking
```

**First run of a new model with the pruned vocab**: pass the pruner source dir as the 5th arg (or `prune_vocab=` to `start_llm`) — the engine's `--prune` gathers the pruned embedding and creates `<model>/vocab/` before loading (idempotent). On the Python side, `start_llm` also copies the tokenizer files into `vocab/` if missing (it needs the tokenizer before the engine runs). `tools/setup_2b.py` wraps the whole 2B prep (verifies tokenizer parity with the 9B's, then runs the pruner).

`vk_llm.py` exposes `start_llm(weight_dir, max_ctx, max_new_tokens, dump_dir=None, dump_layers=0, debug_sampling=False, prune_vocab=None)`, `apply_chat_template(messages, enable_thinking=True)`, `tokenize`, `generate`, `generate_stream`, `detokenize`, `close`. `tokenize(text)` wraps the text in the **Qwen3.5 chat template** with the `<think>` control token (gotcha 32), reading the tokenizer from `<weight_dir>/vocab/tokenizer.json`. EOS comes from the tokenizer's `<|im_end|>` id (85992 in the pruned vocab).
